...
int coin_x = 80, coin_y = 13; //동전 위치, y가 13이 아니면 옆으로 안감..
...
int main() {
	...
	while (1)
	{
		...
		if (t % 30 == 0)
		{
			...
			if (obsOrZombie == 7) showZombie(zombieModel[obs_id]); // 0.125 확률로 장애물 대신 좀비 생성
...
int showZombie(char zombieModel[4][5][3])
{
	...
	for (int y = 1; y < zombie_size_y; y++)
	{
		...
		{
			SetCurrentCursorPos(curPos.X + x, curPos.Y + y + 3); //3을 더해주어야 공중에 안뜹니다.

			if (strcmp(zombieModel[y][x], " ") != 0) {
				printf("%s", zombieModel[y][x]);
				updateGameBoard(curPos.X + x, curPos.Y + y + 3, 2); //y에 3을 더해야 히트박스가 좀비의 위치로 내려옵니다. 3안더하면 좀비의 3칸위에 히트박스가 있음
			}
		}
	}
...
int removeZombie(char zombieModel[4][5][3])
{
	...
	for (int y = 1; y < zombie_size_y; y++)
	{
		for (int x = 0; x < zombie_size_x; x++)
		{
			SetCurrentCursorPos(curPos.X + (x * 1), curPos.Y + y + 3);//3을 더해주어야 공중에 안뜹니다.
			if (strcmp(zombieModel[y][x], " ") != 0) {
				printf(" ");
				updateGameBoard(curPos.X + (x * 1), curPos.Y + y + 3, 0); //y에 3을 더해야 히트박스가 좀비의 위치로 내려옵니다. 3안더하면 좀비의 3칸위에 히트박스가 있음
...
int drawCoin(char coinModel[2][1][2])
{
	...
	for (int y = 1; y < coin_size_y; y++) {
		for (int x = 0; x < coin_size_x; x++) {
			SetCurrentCursorPos(curPos.X + x, curPos.Y + y + 2); //위치 13
